...
def cutout(image, mask, objects, pad_size, replace=0):
  ...
  pad_size = tf.cast(pad_size, tf.float32)
  pad_size = tf.cond(pad_size < 1., lambda:tf.math.sqrt(tf.cast(image_height*image_width,tf.float32))*pad_size, lambda:pad_size)
